...
const serviceTypes = [
    client_1.ServiceType.PERSONAL_TRAINING, client_1.ServiceType.NUTRITION_PLAN, client_1.ServiceType.PHYSIOTHERAPY, client_1.ServiceType.CONSULTATION,
];
...
function randomEmail(name, suffix) {
...
async function main() {
    ...
    for (const ad of adminData) {
        ...
        if (!existing) {
            const a = await prisma.user.create({
                data: { name: ad.name, email: ad.email, passwordHash, role: client_1.UserRole.ADMIN, isActive: true, emailVerified: true },
            });
            admins.push(a);
            console.log(`  ✅ ${ad.name}`);
        }
    ...
    for (let i = 0; i < 30; i++) {
        ...
        for (let g = 0; g < numGyms; g++) {
            ...
            if (!existingGym) {
                ...
                for (let ci = 0; ci < shuffledClasses.length; ci++) {
                    ...
                    const scheduledAt = new Date(baseDate.getTime() + ci * 24 * 60 * 60 * 1000);
                    await prisma.class.create({
                        data: {
                            gymId: gym.id,
                            title: classTitle,
                            description: `Clase de ${classTitle} en ${gymName}. Todos los niveles bienvenidos.`,
                            classType: pick(['IN_PERSON', 'ONLINE', 'HYBRID']),
                            capacity: rnd(10, 30),
        ...
        if ((i + 1) % 5 === 0)
            console.log(`  ✅ ${i + 1}/30 dueños creados...`);
    }
    ...
    for (let i = 0; i < 20; i++) {
        ...
        if ((i + 1) % 5 === 0)
            console.log(`  ✅ ${i + 1}/20 coaches creados...`);
    }
    ...
    for (let i = 0; i < 70; i++) {
        ...
        if ((i + 1) % 10 === 0)
            console.log(`  ✅ ${i + 1}/70 atletas creados...`);
    }
...
//# sourceMappingURL=seed-demo-mario.js.map