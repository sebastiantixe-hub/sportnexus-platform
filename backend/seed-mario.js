...
async function main() {
  console.log('HERCIX Seed Mario - Iniciando...');
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if ((i+1) % 5 === 0) console.log(`  ${i+1}/30 owners...`);
  }
  ...
  for (let i = 0; i < 20; i++) {
    ...
    if ((i+1) % 5 === 0) console.log(`  ${i+1}/20 coaches...`);
  }
  ...
  for (let i = 0; i < 70; i++) {
    ...
    if ((i+1) % 10 === 0) console.log(`  ${i+1}/70 atletas...`);
  }

  const tu = await prisma.user.count();
  const tg = await prisma.gym.count();
  const tc = await prisma.class.count();
  console.log(`\nSEED COMPLETADO: ${tu} usuarios, ${tg} gimnasios, ${tc} clases`);
  console.log('Password de todas las cuentas demo: Hercix2026!');
}
